...
git clone ssh://git@git.visualweber.net:1506/viweb-dev/gamagroup.git -b develop
...
cd /home/dev/htdocs/gamagroup/
git init
git remote add origin ssh://git@git.visualweber.net:1506/viweb-dev/gamagroup.git
git fetch --all
...
```
<VirtualHost *:80>
    ServerAdmin admin@gamagroup.dev.gistensal.com
    ServerName gamagroup.dev.gistensal.com
    ServerAlias www.gamagroup.dev.gistensal.com
    SetEnv APPLICATION_ENV "development"
    SetEnv ENVIRONMENT "development"
    SetEnv ENV "development"

    ErrorLog "/home/dev/logs/gamagroup-error_log"
    DocumentRoot "/home/dev/htdocs/gamagroup/"
    <Directory "/home/dev/htdocs/gamagroup/">
...
##### Domain: http://gamagroup.dev.gistensal.com or https://gamagroup.dev.gistensal.com
...
`/home/dev/htdocs/gamagroup/`

Just run this file, then you will have the delivery files

```
$ bash /home/dev/htdocs/gamagroup/bin/delivery.sh
```

- http://gamagroup.dev.gistensal.com/gamagroup.zip 
- http://gamagroup.dev.gistensal.com/gamagroup.sql 
- Done!
...
- Mysql DB Name: dev_gamagroup
- Mysql DB User: userdb.dev.gamagroup
- Mysql DB Password: VoQApR77gt1KWABk
```
##### MongoDB Database:

```
- Mongo DB Name: dev_gamagroup
- Mongo DB User: userdb.dev.gamagroup
- Mongo DB Password: VoQApR77gt1KWABk
```
...
```
/usr/bin/certbot delete --cert-name gamagroup.dev.gistensal.com
```
...
```
sudo certbot --authenticator standalone --installer nginx -d gamagroup.dev.gistensal.com -d www.gamagroup.dev.gistensal.com --pre-hook "service nginx stop" --post-hook "service nginx start"
```
...
IMPORTANT NOTES:
 - Congratulations! Your certificate and chain have been saved at
   /etc/letsencrypt/live/gamagroup.dev.gistensal.com/fullchain.pem. Your cert will
...
```
zip -r gamagroup.zip . -x \*.buildpath/\* \*.idea/\* \*.project/\* \*nbproject/\* \*.git/\* \*.svn/\* \*.gitignore\* \*.gitattributes\* \*.md \*.MD \*.log \*.zip \*.tar.gz \*.gz \*.tar \*.rar \*.DS_Store \*.lock \*desktop.ini vhost-nginx.conf \*.tmp \*.bat bin/delivery.sh bin/remove-botble.sh readme.html composer.lock wp-config.secure.php

-- without symlink
zip -r4uy gamagroup.zip . -x \*.buildpath/\* \*.idea/\* \*.project/\* \*nbproject/\* \*.git/\* \*.svn/\* \*.gitignore\* \*.gitattributes\* \*.md \*.MD \*.log \*.zip \*.tar.gz \*.gz \*.tar \*.rar \*.DS_Store \*.lock \*desktop.ini vhost-nginx.conf \*.tmp \*.bat bin/delivery.sh bin/remove-botble.sh readme.html composer.lock wp-config.secure.php
```
...
```
mysqldump  -hlocalhost -uuserdb.dev.gamagroup -pVoQApR77gt1KWABk dev_gamagroup > dev_gamagroup.sql
mysqldump  -uuserdb.dev.gamagroup -p dev_gamagroup > dev_gamagroup.sql
mysqldump  -uuserdb.dev.gamagroup -pVoQApR77gt1KWABk dev_gamagroup | gzip -9 > dev_gamagroup.sql.gz
mysqldump  -uuserdb.dev.gamagroup -pVoQApR77gt1KWABk --default-character-set utf8 dev_gamagroup  > dev_gamagroup.mysql
```

* Restoring your MySQL Database

```
mysql -uuserdb.dev.gamagroup -pVoQApR77gt1KWABk dev_gamagroup < dev_gamagroup.sql
mysql -h127.0.0.1 -uuserdb.dev.gamagroup -pVoQApR77gt1KWABk --default-character-set utf8 dev_gamagroup < dev_gamagroup.sql


If you need to restore a database that already exists, you'll need to use mysqlimport command. The syntax for mysqlimport is as follows
mysqlimport -uuserdb.dev.gamagroup -pVoQApR77gt1KWABk dev_gamagroup < dev_gamagroup.sql

To restore compressed backup files you can do the following:
gunzip < dev_gamagroup.sql.gz | mysql -uuserdb.dev.gamagroup -pVoQApR77gt1KWABk dev_gamagroup

Restore from FILE:
mysql.exe --defaults-file="c:\users\anonym~1\appdata\local\temp\tmpn3ytli.cnf"  --protocol=tcp --host=127.0.0.1 --user=userdb.dev.gamagroup --port=3306 --default-character-set=utf8 --comments  < "E:\\dev_gamagroup.sql"
...
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = dev_gamagroup;
```
...
sudo mongodump --host localhost:27017 -u userdb.dev.gamagroup -p VoQApR77gt1KWABk --db dev_gamagroup --out mongodb-2017-04-09.json
```

* Restore your MongoDB Database

```
sudo mongorestore --host localhost:27017 -u userdb.dev.gamagroup -p VoQApR77gt1KWABk --db dev_gamagroup --drop /home/jobsin/mongodb-2017-04-09.json/dev_gamagroup
```
...
$ use dev_gamagroup
$ db.createUser( { user: "userdb.dev.gamagroup",pwd: "VoQApR77gt1KWABk",roles: [ {role:"readWrite",db:"dev_gamagroup"}] } )
```
